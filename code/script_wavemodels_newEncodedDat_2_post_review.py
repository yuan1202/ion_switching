    ...
    for epc in range(EPOCHS):
        ...
        print(
            'Epoch {:03d}/{:03d} - Mean training loss {:.6f}; Mean training F1 {:.6f}; Mean validation loss {:.6f}; Mean validation F1 {:.6f}; Learning rate {:.6f};'.format(
                epc + 1, EPOCHS, np.mean(epoch_trn_losses), f1_trn, np.mean(epoch_vld_losses), f1_vld, cur_lr,
            )
        )
...
for fld, (ndcs_trn, ndcs_vld) in enumerate(skf.split(series_dat_all, skf_trgt)):
    ...
    # setup fold model
    mdl = WaveRNN_Classifier(series_dat_all.shape[-1]).to(DEVICE)
    ...
    schdlr = torch.optim.lr_scheduler.ReduceLROnPlateau(optimzr, factor=.5)
    ...
    fold_train_validate(
        ...
        fold_number=fld,
        save_path='./saved_models/wave_net_model_new_encoded_feats_s{}_w{}_fold{:03d}_checkpoint.pth'.format(step, wndw, fld),
        early_stopping=50
    )
...
for fld in range(15):
    ...
    fld_weight = torch.load('./saved_models/wave_net_model_new_encoded_feats_s{}_w{}_fold{:03d}_checkpoint.pth'.format(step, wndw, fld))
