...
N_FOLDS = 5
rskf = RepeatedStratifiedKFold(n_splits=N_FOLDS, n_repeats=2, random_state=SEED)
...
for fld, (ndcs_trn, ndcs_vld) in enumerate(rskf.split(series_dat_all, skf_trgt)):
    ...
    fold_train_validate(
        ...
        fold_number=fld,
        save_path='./saved_models/wave_RNN_model_final_feats_w500_fold{:03d}_checkpoint.pth',
        early_stopping=50
    )
